...
public class MainGameFrame extends JFrame {
    ...
    ImageIcon basicIconMyCity, attackerIconMyCity, explorerIconMyCity,
            rusherIconMyCity;
    JLabel numBasicMyCity, numAttackerMyCity, numExplorerMyCity,
            numRusherMyCity;
    ...
    public MainGameFrame(Grid grid) {
        ...
        for (int i = 0; i < ROWS; i++) {
            for (int j = 0; j < COLS; j++) {
                ...
                if (mapTiles.getTile(new Coord(i, j)) != null)
                    gridSquares[i][j].setBackground(stringToColor(mapTiles
                            .getTile(new Coord(i, j)).getColor()));
                else {
                    // mapTiles[i][j] is a city and I need to pick the image of
                    // the right
                    // color
                }
                gridSquares[i][j].addMouseListener(gridSquareMouseListener);
                gridSquares[i][j].setPreferredSize(new Dimension(SQUARESIZE,
                        SQUARESIZE));
        ...
        narrationPanel.getVerticalScrollBar().setPreferredSize(
                new Dimension(15, 0));
        ...
        // ************************CITY SELECT HUD***************************
        cityStatus_CITY = new JLabel("City is occupied by Jordan",
                JLabel.CENTER);
        ...
        public void mouseClicked(MouseEvent e) {
            if (currentHUDCard != "Lock") {
                            ...
                            if (prevSelected == null
                                ...
                                if (currentSelected.getOccupant() != null
                                    ...
                                    if (currentSelected.owner == -1) {
                                        ...
                                        cityMainLayout
                                                .show(HUDCitySelectMainPanel,
                                                        "enemy");
                                        currentHUDCard = "City";
                                    }
                                } else if (currentSelected.isResource()) {// is
                                                                          // a
                                                                          // resource
                                ...
                                currentSelected = mapTiles.getTile(new Coord(i,
                                        j));
                                if (prevSelected == currentSelected) {// clicked
                                                                      // again
                                    currentSelected = null;
                                } else if (currentSelected.getOccupant() != null) {// occupied
                                    // by army
                                    if (currentSelected.getOccupant()
                                            .getColor() == prevSelected
                                        ...
                                        System.out
                                                .println(name
                                                        ...
                                                        + prevSelected.coord
                                                        + "attacks "
                                                        + Siege.players[currentSelected
                                                                .getOccupant().owner].name
                                                        + "army at "
                                                        + currentSelected.coord);
                                        try {
                                            Siege.sendToServer(name
                ...
                if (SwingUtilities.isLeftMouseButton(e)) {
                    ...
                    System.out.println("left clicked on square at: (Row: "
                            + rowNumber + " Col: " + colNumber + ")");
                    if (currentSelected != null)
                    ...
                    if (prevSelected != null)
                        System.out.println("prevSelected: "
                                + prevSelected.coord);
                    else
                ...
                if (SwingUtilities.isRightMouseButton(e)) {
                    System.out.println("right clicked on square at: (Row: "
                            + rowNumber + " Col: " + colNumber + ")");
            ...
            for (Integer i = 0; i < ROWS; i++) {
                for (Integer j = 0; j < COLS; j++) {
                    if (e.getSource() == gridSquares[i][j]) {
                        ...
                        if (currentTile.owner == -1)
                            gridSquares[i][j]
                                    .setBackground(stringToColor(mapTiles
    ...
    void updateGridSquare(Coord pos) {
        ...
        gridSquares[pos.row][pos.col].setBackground(stringToColor(update
                .getColor()));
        ...
        if (update.isCity() == false && update.getOccupant() != null) {
            ...
            try {
                armyImage = ImageIO.read(new File("src/resources/"
                        + occupantColor + "Army.png"));
            ...
            if (strength != -1) {
                ...
                strLabel.setFont(new Font(Font.MONOSPACED, Font.BOLD,
                        SQUARESIZE / 2));
        ...
        else if (update.isResource()) {// resource or none
    ...
    void outlineAroundSquare(Integer row, Integer col, Integer radius) {
        gridSquares[row][col].setBorder(BorderFactory
    ...
    void printResourcePanel(Tile resourceTile) {
        ...
        resourceBonus
                .setText("Resource adds " + resourceTile.income + "income");
    }

    void printLockedPanel() {
        HUDLockedPanel.add(new JLabel("Currently "
                + Siege.players[Siege.currentPlayer].name + "'s turn",
    ...
    String guessResourceBackground(Tile res) {
        ...
        if (res.coord.row > 0) {// not top
            if (mapTiles.getTile(new Coord(res.coord.row - 1, res.coord.col))
                    .getColor() == "forest")
                forest++;
            else if (mapTiles.getTile(
                    new Coord(res.coord.row - 1, res.coord.col)).getColor() == "mountain")
                mountain++;
            else if (mapTiles.getTile(
                    new Coord(res.coord.row - 1, res.coord.col)).getColor() == "muddy")
                muddy++;
            else
                plains++;
        }
        ...
        if (res.coord.col > 0) {// not left
            if (mapTiles.getTile(new Coord(res.coord.row, res.coord.col - 1))
                    .getColor() == "forest")
                forest++;
            else if (mapTiles.getTile(
                    new Coord(res.coord.row, res.coord.col - 1)).getColor() == "mountain")
                mountain++;
            else if (mapTiles.getTile(
                    new Coord(res.coord.row, res.coord.col - 1)).getColor() == "muddy")
                muddy++;
            else
                plains++;
        }
